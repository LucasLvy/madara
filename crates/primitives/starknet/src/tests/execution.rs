...
fn test_call_entry_point_execute_works() {
    ...
    let class_hash = U256::from_str_radix(TEST_CLASS_HASH, 16).unwrap();
    let address = U256::from_str_radix(TEST_CONTRACT_ADDRESS, 16).unwrap();
    let selector = U256::from_big_endian(&selector_from_name("return_result").0.0);
    ...
    assert_ok!(entrypoint.execute(&mut test_state, block, U256::zero(), chain_id));
}
...
fn test_call_entry_point_execute_fails_undeclared_class_hash() {
    ...
    assert!(entrypoint.execute(&mut test_state, block, U256::zero(), chain_id).is_err());
}
...
fn test_try_into_entrypoint_works() {
    let entrypoint_wrapper = CallEntryPointWrapper {
        class_hash: Some(U256::from_str_radix("0x1", 16).unwrap()),
        ...
        storage_address: U256::from_str_radix("0x1", 16).unwrap(),
        caller_address: U256::from_str_radix("0x2", 16).unwrap(),
    };
    let entrypoint: CallEntryPoint = entrypoint_wrapper.try_into().unwrap();
