    ...
    new_test_ext().execute_with(|| {
        ...
        let transaction = DeployAccountTransaction {
            ...
            calldata: BoundedVec::try_from(
                calldata.clone().into_iter().map(|e| U256::from_str_radix(e, 16).unwrap()).collect::<Vec<U256>>(),
            )
            .unwrap(),
            nonce: U256::zero(),
        ...
        let expected_fee_transfer_event = Event::StarknetEvent(EventWrapper {
            keys: bounded_vec![
                U256::from_str_radix("0x0099cd8bde557814842a3121e8ddfd433a539b8c9f14bf31ebf108d12e6196e9", 16).unwrap()
            ],
            data: bounded_vec!(
                test_addr,                                   // From
                U256::from_str_radix("0x2", 16).unwrap(),    // To
                U256::from_str_radix("0xd3b8", 16).unwrap(), // Amount low
...
fn given_contract_run_deploy_account_tx_undeclared_then_it_fails() {
    new_test_ext().execute_with(|| {
        ...
        let salt = "0x03b37cbe4e9eac89d54c5f7cc6329a63a63e8c8db2bf936f981041e086752463";
        let none_origin = RuntimeOrigin::none();
        let rand_address = U256::from_str_radix("0x1234", 16).unwrap();
...
fn given_contract_run_deploy_account_openzeppelin_tx_works() {
    new_test_ext().execute_with(|| {
        ...
        let tx_hash =
            U256::from_str_radix("0x06ff0e0245daed20c0b4f21ae5c9286ba3a03e0c62b2bec2d0dcec2a4d6b9889", 16).unwrap();
...
fn given_contract_run_deploy_account_openzeppelin_with_incorrect_signature_then_it_fails() {
    new_test_ext().execute_with(|| {
        ...
        let transaction = DeployAccountTransaction {
            ...
            nonce: U256::zero(),
            max_fee: U256::from(u128::MAX),
            signature: bounded_vec!(U256::one(), U256::one()),
        };
...
fn given_contract_run_deploy_account_argent_tx_works() {
    new_test_ext().execute_with(|| {
        ...
        let tx_hash =
            U256::from_str_radix("0x0781152a4f3fc0dada10f24a40f7499ce3c17c3867acae82024f5507475f89da", 16).unwrap();
...
fn given_contract_run_deploy_account_braavos_tx_works() {
    new_test_ext().execute_with(|| {
        ...
        let tx_hash =
            U256::from_str_radix("0x06ae3d81978d498def89e1121b2d84a873d63c30d80f7ed81e2dc9be6a961770", 16).unwrap();

        let mut signatures: Vec<U256> = sign_message_hash(tx_hash).into();
        let empty_signatures = [U256::zero(); 8];
...
fn given_contract_run_deploy_account_braavos_with_incorrect_signature_then_it_fails() {
    new_test_ext().execute_with(|| {
        ...
        let transaction = DeployAccountTransaction {
            ...
            nonce: U256::zero(),
            max_fee: U256::from(u128::MAX),
            signature: [U256::zero(); 10].to_vec().try_into().unwrap(),
        };
...
fn set_infinite_tokens(address: U256) {
...
fn set_signer(address: U256, account_type: AccountType) {
    let (var_name, args) = match account_type {
        AccountType::Argent => ("_signer", vec![]),
        AccountType::Braavos => ("Account_signers", vec![U256::zero()]),
