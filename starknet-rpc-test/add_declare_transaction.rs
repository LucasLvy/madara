...
// use starknet_providers::{MaybeUnknownErrorCode, Provider, ProviderError,
// StarknetErrorWithMessage}; use starknet_rpc_test::constants::{ARGENT_CONTRACT_ADDRESS,
// FEE_TOKEN_ADDRESS, OZ_CONTRACT_ADDRESS, SIGNER_PRIVATE};
// use starknet_rpc_test::fixtures::{madara, ThreadSafeMadaraClient};
// use starknet_rpc_test::utils::{build_single_owner_account, read_erc20_balance, AccountActions,
// U256}; use starknet_rpc_test::{SendTransactionError, Transaction, TransactionResult};
...
//         let (declare_tx, _, _) =
//             account.declare_contract("./contracts/Counter.sierra.json",
// "./contracts/Counter.casm.json");
...
// #[rstest]
// #[tokio::test]
// async fn fail_execution_step_with_no_storage_change(madara: &ThreadSafeMadaraClient) ->
// Result<(), anyhow::Error> {     let rpc = madara.get_starknet_client().await;
...
//         .declare_contract("./contracts/Counter5/Counter5.sierra.json",
// "./contracts/Counter5/Counter5.casm.json");

//     let (block_number, txs) = {
//         let mut madara_write_lock = madara.write().await;
//         // draining oz_account so the txn fails during execution
//         let balance =
//             read_erc20_balance(&rpc, FieldElement::from_hex_be(FEE_TOKEN_ADDRESS).unwrap(),
// oz_account.address()).await;         madara_write_lock
...
//                 U256 { low: balance[0] - FieldElement::from_dec_str("150000").unwrap(), high:
// balance[1] },                 None,
//             ))])
//             .await?;
...
// true);     let (declare_tx, expected_class_hash, _) = account
//         .declare_contract("./contracts/Counter1/Counter1.sierra.json",
// "./contracts/Counter1/Counter1.casm.json");
...
//         Ok(TransactionResult::Declaration(DeclareTransactionResult { transaction_hash: _,
// class_hash })) => {             assert_eq!(class_hash, expected_class_hash);
//         }
...
//     let account = build_single_owner_account(&rpc, SIGNER_PRIVATE, ARGENT_CONTRACT_ADDRESS,
// true);     let (declare_tx, _, _) = account
//         .declare_contract("./contracts/Counter2/Counter2.sierra.json",
// "./contracts/Counter2/Counter2.casm.json");
